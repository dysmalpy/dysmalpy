...
class TestFittingWrappers:
    ...
    def check_output_files(self, outdir, list_files):
    ...
    def check_bestfit_values(self, results, dict_bf_values, fit_method=None, ndim=None,
                    ftol=1.e-3):
        for compn in dict_bf_values.keys():
            for paramn in dict_bf_values[compn].keys():
                ...
                if len(whm) == 1:
                    fracdel = (np.float(results['best_value'].iloc[whm[0]])-dict_bf_values[compn][paramn])/dict_bf_values[compn][paramn]
                    assert fracdel <= ftol, '{}, ndim={}, {}:{}'.format(fit_method,ndim,compn,paramn)
                else:
                    raise ValueError('{}, ndim={}, {}:{}'.format(fit_method,ndim,compn,paramn))


    def expected_output_files_base(self, galID, param_filename=None, fit_method=None):
    ...
    def expected_output_files_1D(self, galID, param_filename=None, fit_method=None):
        list_files = self.expected_output_files_base(galID,
                            param_filename=param_filename, fit_method=fit_method)
    ...
    def expected_output_files_2D(self, galID, param_filename=None, fit_method=None):
        list_files = self.expected_output_files_base(galID,
                            param_filename=param_filename, fit_method=fit_method)
    ...
    def expected_output_files_3D(self, galID, param_filename=None, fit_method=None):
        list_files = self.expected_output_files_base(galID,
                            param_filename=param_filename, fit_method=fit_method)
    ...
    def test_1D_mpfit(self):
        ...
        params = self.read_params(param_filename=param_filename)
        outdir_full = _dir_tests_data+params['outdir']

        self.run_fit(param_filename=param_filename)

        # Make sure all files exist:
        list_files = self.expected_output_files_1D(params['galID'], param_filename=param_filename,
                            fit_method=params['fit_method'])
        self.check_output_files(outdir_full, list_files)
        ...
        self.check_bestfit_values(results, dict_bf_values,
                            fit_method=params['fit_method'], ndim=1)
    ...
    def test_2D_mpfit(self):
        ...
        # Make sure all files exist:
        list_files = self.expected_output_files_2D(params['galID'], param_filename=param_filename,
                            fit_method=params['fit_method'])
        self.check_output_files(outdir_full, list_files)
        ...
        self.check_bestfit_values(results, dict_bf_values,
                            fit_method=params['fit_method'], ndim=2)
    ...
    def test_3D_mpfit(self):
        ...
        # Make sure all files exist:
        list_files = self.expected_output_files_3D(params['galID'], param_filename=param_filename,
                            fit_method=params['fit_method'])
        self.check_output_files(outdir_full, list_files)
        ...
        self.check_bestfit_values(results, dict_bf_values,
                            fit_method=params['fit_method'], ndim=3)
