...
class TwoPowerHalo(DarkMatterHalo):
    ...
    #             mtest = np.arange(-5, 50, 1.0)
    #             if adiabatic_contract:
    #                 vtest = np.array([self._minfunc_vdm_mvir_from_fdm_AC(m, vsqr_dm_re_target, self.conc.value,
    #                                                 self.alpha.value, self.beta.value, self.z, r_fdm, baryons) for m in mtest])
    #                 # TEST
    #                 vtest_noAC = np.array([self._minfunc_vdm_mvir_from_fdm(m, vsqr_dm_re_target, self.conc.value,
    #                                                 self.alpha.value, self.beta.value, self.z, r_fdm) for m in mtest])
    #             else:
    #                 vtest = np.array([self._minfunc_vdm_mvir_from_fdm(m, vsqr_dm_re_target, self.conc.value,
    #                                                 self.alpha.value, self.beta.value, self.z, r_fdm) for m in mtest])
    #             try:
    #                 a = mtest[vtest < 0][-1]
    #                 b = mtest[vtest > 0][0]
    ...
    #
    #             if adiabatic_contract:
    #                 mvirial = scp_opt.brentq(self._minfunc_vdm_mvir_from_fdm_AC, a, b, args=(vsqr_dm_re_target, self.conc.value,
    #                                                     self.alpha.value, self.beta.value, self.z, r_fdm, baryons))
    #
    #                 # TEST
    #                 mvirial_noAC = scp_opt.brentq(self._minfunc_vdm_mvir_from_fdm, a_noAC, b_noAC, args=(vsqr_dm_re_target, self.conc.value,
    #                                                     self.alpha.value, self.beta.value, self.z, r_fdm))
    #                 print("mvirial={}, mvirial_noAC={}".format(mvirial, mvirial_noAC))
    #             else:
    #                 mvirial = scp_opt.brentq(self._minfunc_vdm_mvir_from_fdm, a, b, args=(vsqr_dm_re_target, self.conc.value,
    #                                                     self.alpha.value, self.beta.value, self.z, r_fdm))
    #     return mvirial
    #
    # def _minfunc_vdm_mvir_from_fdm(self, mvirial, vtarget, conc, alpha, beta, z, r_eff):
    #     halo = TwoPowerHalo(mvirial=mvirial, conc=conc, alpha=alpha, beta=beta, z=z)
    #     return halo.circular_velocity(r_eff) ** 2 - vtarget
    #
    # def _minfunc_vdm_mvir_from_fdm_AC(self, mvirial, vtarget, conc, alpha, beta, z, r_eff, bary):
    #     halo = TwoPowerHalo(mvirial=mvirial, conc=conc, alpha=alpha, beta=beta, z=z)
    #     modtmp = ModelSet()
    #     modtmp.add_component(bary, light=True)
...
class NFW(DarkMatterHalo):
    ...
    #             mtest = np.arange(-5, 50, 1.0)
    #             if adiabatic_contract:
    #                 vtest = np.array([self._minfunc_vdm_mvir_from_fdm_AC(m, vsqr_dm_re_target, self.conc.value, self.z, r_fdm, baryons) for m in mtest])
    #                 # TEST
    #                 vtest_noAC = np.array([self._minfunc_vdm_mvir_from_fdm(m, vsqr_dm_re_target, self.conc.value, self.z, r_fdm) for m in mtest])
    #             else:
    #                 vtest = np.array([self._minfunc_vdm_mvir_from_fdm(m, vsqr_dm_re_target, self.conc.value, self.z, r_fdm) for m in mtest])
    #
    #             try:
    #                 a = mtest[vtest < 0][-1]
    #                 b = mtest[vtest > 0][0]
    ...
    #
    #             if adiabatic_contract:
    #                 mvirial = scp_opt.brentq(self._minfunc_vdm_mvir_from_fdm_AC, a, b, args=(vsqr_dm_re_target, self.conc.value, self.z, r_fdm, baryons))
    #
    #                 # TEST
    #                 mvirial_noAC = scp_opt.brentq(self._minfunc_vdm_mvir_from_fdm, a_noAC, b_noAC, args=(vsqr_dm_re_target, self.conc.value, self.z, r_fdm))
    #                 print("mvirial={}, mvirial_noAC={}".format(mvirial, mvirial_noAC))
    #             else:
    #                 mvirial = scp_opt.brentq(self._minfunc_vdm_mvir_from_fdm, a, b, args=(vsqr_dm_re_target, self.conc.value, self.z, r_fdm))
    #
    #     return mvirial
    #
    # def _minfunc_vdm_mvir_from_fdm(self, mass, vtarget, conc, z, r_eff):
    #     halo = NFW(mvirial=mass, conc=conc, z=z)
    #     return halo.circular_velocity(r_eff) ** 2 - vtarget
    #
    # def _minfunc_vdm_mvir_from_fdm_AC(self, mass, vtarget, conc, z, r_eff, bary):
